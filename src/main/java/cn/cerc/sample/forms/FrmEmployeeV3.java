...
public class FrmEmployeeV3 extends CustomForm {
    ...
    public IPage execute() throws ServiceExecuteException {
        ...
        // 调用查询服务
        DataSet dataOut = ServiceQuery.open(this, SvrEmployeeV3.search, search.current()).getDataOutElseThrow();
    ...
    public IPage append() {
        ...
        if (!Utils.isEmpty(actionForm.readAll())) {
            ServiceQuery svr = ServiceQuery.open(this, SvrEmployeeV3.append, actionForm.getRecord());
            if (svr.isFail())
    ...
    public IPage modify() {
        ...
        try (MemoryBuffer buff = new MemoryBuffer(BufferUser.Notice_UserCode, this.getUserCode(),
            ...
            // 判断是否用户有点击保存按钮，若有则读取所有的提交到actionForm.getRecord()
            if (!Utils.isEmpty(actionForm.readAll())) {
                ServiceQuery svr = ServiceQuery.open(this, SvrEmployeeV3.modify, actionForm.getRecord());
                if (svr.isFail())
                    return page.setMessage(svr.dataOut().message());
                // 传递成功的消息到修改页面
